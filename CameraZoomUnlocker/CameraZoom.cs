...
namespace CameraZoomUnlocker
{
    ...
    static class Main
    {
        ...
        static void OnShowGui(UnityModManager.ModEntry modEntry)
        {
            try
            {
                if (CameraControl.Instance?.GetZoomLevel() != null)
                {
                    settings.CurrentZoom = CameraControl.Instance.GetZoomLevel();
                }
            }
            catch (Exception ex)
            {
        ...
        static public void SaveZoomLevel()
        {
            try
            {
                if (settings != null)
                {
                    settings.CurrentZoom = CameraControl.Instance?.GetZoomLevel() ?? 1.0f;
                }
            }
            catch (Exception ex)
            {
        ...
        static public void SetZoomLevel()
        {
            try
            {
                if (!enabled && CameraControl.Instance != null)
                {
                    if (GameState.Option != null)
                    {
                        ...
                        CameraControl.Instance.SetZoomLevel(Settings.DefaultZoomLevel);
                    }
                }
                else
                {
                        ...
                        GameState.Option != null)
                    {
                        ...
                        CameraControl.Instance.SetZoomLevel(settings.CurrentZoom);
                    }
                }
            }
            catch (Exception ex)
