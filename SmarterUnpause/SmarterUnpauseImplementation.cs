...
namespace SmarterUnpause
{
    ...
    static class AutoPause_New_Prefix
    {
        static void Prefix()
        {
            try
            {
                if (Main.enabled)
                {
                    SmarterUnpauseManager.WasPaused = TimeController.Instance.Paused;
                }
            }
            catch (Exception ex)
            {
    ...
    static class AutoPause_New_Postfix
    {
        static void Postfix()
        {
            try
            {
                if (Main.enabled && !SmarterUnpauseManager.WasPaused && TimeController.Instance.Paused)
                {
                    // auto-pause happened; remember real time
                    SmarterUnpauseManager.AutoPauseTime = Time.realtimeSinceStartup;
                }
            }
            catch (Exception ex)
            {
    ...
    [HarmonyPatch(typeof(UIActionBarOnClick), "HandlePause", MethodType.Normal)]
    static class UIActionBarOnClick_New
    {
        static bool Prefix()
        {
            try
            {
                if (Main.enabled)
                {
                    float timeSinceAutoPause = Time.realtimeSinceStartup;

                    if (TimeController.Instance.Paused && timeSinceAutoPause < Main.settings.MillisecondsAsFloat)
                        // if user presses unpause again, it should not be suppressed
                        SmarterUnpauseManager.AutoPauseTime = 0.0f;
                    else
                        // Unpause
                        TimeController.Instance.Paused = !TimeController.Instance.Paused;
